...
## I/O parameters.
stan_model = 'TDLR'
PATH_NAME = '/usr/people/kmarr/THESIS/TDLR.stan'
...
for subject in data['subject'].unique():
    ...
    for i in tqdm(range(T)):
        ...
        theta = inv_logit( beta1 * np.diff(Q[:,0], axis=-1).reshape((n_samp,)) )
...
total_data.to_csv("all_PPC_TDLR.csv")

total_waic = concat(total_waic)
total_waic.to_csv("all_TDLR_MC.csv")

sums = concat(sums)
sums.to_csv("all_PPC_TDLR_stanfit.csv")