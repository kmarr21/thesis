...
## I/O parameters.
stan_model = 'n1EAPFUL'
PATH_NAME = '/usr/people/kmarr/THESIS/n1EAPFUL'
...
for subject in data['subject'].unique():
    ...
    for i in tqdm(range(T)):
        ...
        # Chosen action
        deltaC = R[i] - Q1[:, Y1[i]]
        Q1[:, Y1[i]] += eta_1 * lam * (w * cr * deltaC + (1-w) * deltaC)
        # Unchosen action
        deltaU = R[i] - Q1[:, 1-Y1[i]]
        Q1[:, 1-Y1[i]] += eta_1 * lam * w * (1-cr) * deltaU
...
total_data.to_csv("all_PPC_n1EAPFUL.csv")

total_waic = concat(total_waic)
total_waic.to_csv("all_n1EAPFUL_MC.csv")

sums = concat(sums)
sums.to_csv("all_PPC_n1EAPFUL_stanfit.csv")