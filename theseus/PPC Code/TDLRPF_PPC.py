...
## I/O parameters.
stan_model = 'TDLRPF'
PATH_NAME = '/usr/people/kmarr/THESIS/TDLRPF.stan'
...
for subject in data['subject'].unique():
    ...
    for i in tqdm(range(T)):
        ...
        ## Forgetting process
        Q[:,1-S2[i]+1,Y2[i]] += etaF * (0 - Q[:,1-S2[i]+1,Y2[i]])
        Q[:,S2[i]+1,1-Y2[i]] += etaF * (0 - Q[:,S2[i]+1,1-Y2[i]])
        Q[:,1-S2[i]+1,1-Y2[i]] += etaF * (0 - Q[:,1-S2[i]+1,1-Y2[i]])
...
total_data.to_csv("all_PPC_TDLRPF.csv")

total_waic = concat(total_waic)
total_waic.to_csv("all_TDLRPF_MC.csv")

sums = concat(sums)
sums.to_csv("all_PPC_TDLRPF_stanfit.csv")