...
## I/O parameters.
stan_model = 'PF_hybrid'
PATH_NAME = '/usr/people/kmarr/THESIS/PF_hybrid'
...
for subject in data['subject'].unique():
    ...
    for i in tqdm(range(T)):
        ...
        ## Forgetting process
        Q[:,1-S2[i],Y2[i]] += eta_F * (0 - Q[:,1-S2[i],Y2[i]])
        Q[:,S2[i],1-Y2[i]] += eta_F * (0 - Q[:,S2[i],1-Y2[i]])
        Q[:,1-S2[i],1-Y2[i]] += eta_F * (0 - Q[:,1-S2[i],1-Y2[i]])
...
total_data.to_csv("all_PPC_PF_hyb.csv")

total_waic = concat(total_waic)
total_waic.to_csv("all_PF_hyb_MC.csv")

sums = concat(sums)
sums.to_csv("all_PPC_PF_hyb_stanfit.csv")