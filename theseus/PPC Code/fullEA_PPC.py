...
## I/O parameters.
stan_model = 'fullEA'
PATH_NAME = '/usr/people/kmarr/THESIS/fullEA'
...
for subject in data['subject'].unique():
    ...
    for i in tqdm(range(T)):
        ...
        ## Forgetting
        Q2[:,1-S2[i],Y2[i]] += eta_F * (u - Q2[:,1-S2[i],Y2[i]])
        Q2[:,S2[i],1-Y2[i]] += eta_F * (u - Q2[:,S2[i],1-Y2[i]])
        Q2[:,1-S2[i],1-Y2[i]] += eta_F * (u - Q2[:,1-S2[i],1-Y2[i]])
        ...
        # Chosen action
        deltaC = R[i] - Q1[:, Y1[i]]
        Q1[:, Y1[i]] += eta_1 * (w * lam * cr * deltaC + (1-w) * lam * deltaC)
        # Unchosen action
        deltaU = R[i] - Q1[:, 1-Y1[i]]
        Q1[:, 1-Y1[i]] += eta_1 * w * lam * (1-cr) * deltaU
...
total_data.to_csv("all_PPC_fullEA.csv")

total_waic = concat(total_waic)
total_waic.to_csv("all_fullEA_MC.csv")

sums = concat(sums)
sums.to_csv("all_PPC_fullEA_stanfit.csv")