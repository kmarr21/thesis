...
## I/O parameters.
stan_model = 'EAPF2'
PATH_NAME = '/usr/people/kmarr/THESIS/EAPF2'
...
for subject in data['subject'].unique():
    ...
    for i in tqdm(range(T)):
        ...
        ## Forgetting
        Q2[:,1-S2[i],Y2[i]] += eta_F * (0 - Q2[:,1-S2[i],Y2[i]])
        Q2[:,S2[i],1-Y2[i]] += eta_F * (0 - Q2[:,S2[i],1-Y2[i]])
        Q2[:,1-S2[i],1-Y2[i]] += eta_F * (0 - Q2[:,1-S2[i],1-Y2[i]])
...
total_data.to_csv("all_PPC_EAPF2.csv")

total_waic = concat(total_waic)
total_waic.to_csv("all_EAPF2_MC.csv")

sums = concat(sums)
sums.to_csv("all_PPC_EAPF2_stanfit.csv")