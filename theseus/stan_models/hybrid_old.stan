...
model {
  ...
  for (i in 1:T) {
    ...
    if (i > 1) {
        ...
        m = 1 ? (S2[i-1] == S2[i]): 0; // ternary conditional operator
        deV1[i] += p*m;
    }

    // update second hybrid values - choice likelihood for level 2
    deV2[i] = Q[2 + (S2[i]*2)] - Q[1 + (S2[i]*2)];
...
generated quantities {
  ...
  // Likelihood block
  {
    ...
    for (i in 1:T) {
      ...
      // Compute hybrid value
      s1_prob = w*0.4*(fmax(Q[4], Q[3]) - fmax(Q[2], Q[1])) + (1-w)*(MF[2] - MF[1]);
      ...
      // Stage 1 choice
      Y1_pd += exp( bernoulli_logit_lpmf( Y1[i] | beta_1 * s1_prob ) );

      // update second hybrid values - choice likelihood for level 2
      s2_prob = Q[2 + (S2[i]*2)] - Q[1 + (S2[i]*2)];
      // Stage 2 choice
