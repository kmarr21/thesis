...
model {    
    ...
    for (i in 1:T) {
        ...
        // Forgetting Process
        Q[S2[i]+1, (1-Y2[i])+1] += eta_F * ( u - Q[S2[i]+1, (1-Y2[i])+1] );
        Q[(1-S2[i])+1, Y2[i]+1] += eta_F * ( u - Q[(1-S2[i])+1, Y2[i]+1] );
        Q[(1-S2[i])+1, (1-Y2[i])+1] += eta_F * ( u - Q[(1-S2[i])+1, (1-Y2[i])+1] );
        ...
        // Chosen action
        deltaC = R[i] - MF[Y1[i] + 1];
        MF[Y1[i] + 1] += eta_1 * (w * cr * deltaC + (1-w) * deltaC);

        // Unchosen action
        deltaU = R[i] - MF[(1-Y1[i]) + 1];
        MF[(1-Y1[i]) + 1] += eta_1 * w * (1 - cr) * deltaU;
